...
function axis(orient, scale2) {
  var tickArguments = [], tickValues = null, tickFormat = null, tickSizeInner = 6, tickSizeOuter = 6, tickPadding = 3, offset = typeof window !== "undefined" && window.devicePixelRatio > 1 ? 0 : 0.5, k = orient === top || orient === left ? -1 : 1, x3 = orient === left || orient === right ? "x" : "y", transform = orient === top || orient === bottom ? translateX : translateY;
  function axis2(context) {
    var values = tickValues == null ? scale2.ticks ? scale2.ticks.apply(scale2, tickArguments) : scale2.domain() : tickValues, format = tickFormat == null ? scale2.tickFormat ? scale2.tickFormat.apply(scale2, tickArguments) : identity_default : tickFormat, spacing = Math.max(tickSizeInner, 0) + tickPadding, range4 = scale2.range(), range0 = +range4[0] + offset, range1 = +range4[range4.length - 1] + offset, position = (scale2.bandwidth ? center : number)(scale2.copy(), offset), selection = context.selection ? context.selection() : context, path2 = selection.selectAll(".domain").data([null]), tick = selection.selectAll(".tick").data(values, scale2).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
    ...
    text.attr(x3, k * spacing).text(format);
...
function brush(dim) {
  var extent2 = defaultExtent, filter = defaultFilter, touchable = defaultTouchable, keys = true, listeners = dispatch_default("start", "brush", "end"), handleSize = 6, touchending;
  ...
  function started(event) {
    ...
    var that = this, type2 = event.target.__data__.type, mode = (keys && event.metaKey ? type2 = "overlay" : type2) === "selection" ? MODE_DRAG : keys && event.altKey ? MODE_CENTER : MODE_HANDLE, signX = dim === Y ? null : signsX[type2], signY = dim === X ? null : signsY[type2], state = local(that), extent3 = state.extent, selection = state.selection, W = extent3[0][0], w0, w1, N = extent3[0][1], n0, n1, E = extent3[1][0], e0, e1, S = extent3[1][1], s0, s1, dx = 0, dy = 0, moving, shifting = signX && signY && keys && event.shiftKey, lockX, lockY, points = Array.from(event.touches || [event], (t) => {
    ...
    function move(event2) {
      ...
      switch (mode) {
        ...
        case MODE_DRAG: {
          ...
          if (signY) dy = max2(N - n0, min(S - s0, dy)), n1 = n0 + dy, s1 = s0 + dy;
          break;
        }
        case MODE_HANDLE: {
          if (points[1]) {
            if (signX) w1 = max2(W, min(E, points[0][0])), e1 = max2(W, min(E, points[1][0])), signX = 1;
            if (signY) n1 = max2(N, min(S, points[0][1])), s1 = max2(N, min(S, points[1][1])), signY = 1;
          } else {
            if (signX < 0) dx = max2(W - w0, min(E - w0, dx)), w1 = w0 + dx, e1 = e0;
            else if (signX > 0) dx = max2(W - e0, min(E - e0, dx)), w1 = w0, e1 = e0 + dx;
            if (signY < 0) dy = max2(N - n0, min(S - n0, dy)), n1 = n0 + dy, s1 = s0;
            else if (signY > 0) dy = max2(N - s0, min(S - s0, dy)), n1 = n0, s1 = s0 + dy;
          }
          break;
        }
        case MODE_CENTER: {
          if (signX) w1 = max2(W, min(E, w0 - dx * signX)), e1 = max2(W, min(E, e0 + dx * signX));
          if (signY) n1 = max2(N, min(S, n0 - dy * signY)), s1 = max2(N, min(S, s0 + dy * signY));
          break;
        }
      }
      if (e1 < w1) {
    ...
    dx = k ? padAngle : tau / n;
    {
      ...
      for (const i of groupIndex) {
        ...
        if (directed) {
          ...
          groups[i] = { index: i, startAngle: x06, endAngle: x3, value: groupSums[i] };
        } else {
...
function ringContains(ring, point) {
  var x3 = point[0], y3 = point[1], contains = -1;
  for (var i = 0, n = ring.length, j = n - 1; i < n; j = i++) {
    var pi3 = ring[i], xi = pi3[0], yi = pi3[1], pj = ring[j], xj = pj[0], yj = pj[1];
    if (segmentContains(pi3, pj, point)) return 0;
...
function contours_default() {
  ...
  function index2(point) {
    return point[0] * 2 + point[1] * (dx + 1) * 4;
  }
  function smoothLinear(ring, values, value) {
    ring.forEach(function(point) {
      var x3 = point[0], y3 = point[1], xt = x3 | 0, yt = y3 | 0, v1 = valid(values[yt * dx + xt]);
      if (x3 > 0 && x3 < dx && xt === x3) {
        point[0] = smooth1(x3, valid(values[yt * dx + xt - 1]), v1, value);
      }
      if (y3 > 0 && y3 < dy && yt === y3) {
        point[1] = smooth1(y3, valid(values[(yt - 1) * dx + xt]), v1, value);
      }
    });
  }
  ...
  contours.thresholds = function(_) {
    return arguments.length ? (threshold = typeof _ === "function" ? _ : Array.isArray(_) ? constant_default3(slice2.call(_)) : constant_default3(_), contours) : threshold;
  };
...
function density_default() {
  var x3 = defaultX, y3 = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k = 2, o = r * 3, n = dx + o * 2 >> k, m3 = dy + o * 2 >> k, threshold = constant_default3(20);
  function grid(data) {
  ...
  function density(data) {
    ...
    return contours_default().size([n, m3]).thresholds(tz.map((d) => d * pow4k))(values).map((c5, i) => (c5.value = +tz[i], transform(c5)));
  }
  density.contours = function(data) {
    var values = grid(data), contours = contours_default().size([n, m3]), pow4k = Math.pow(2, 2 * k), contour = (value) => {
      value = +value;
      var c5 = transform(contours.contour(values, value * pow4k));
      c5.value = value;
      return c5;
    };
  ...
  density.thresholds = function(_) {
    return arguments.length ? (threshold = typeof _ === "function" ? _ : Array.isArray(_) ? constant_default3(slice2.call(_)) : constant_default3(_), density) : threshold;
  };
...
function link_default(links) {
  var id = index, strength = defaultStrength, strengths, distance = constant_default4(30), distances, nodes, count2, bias, random, iterations2 = 1;
  if (links == null) links = [];
...
function circleStream(stream, radius, delta, direction, t0, t1) {
  ...
  if (t0 == null) {
    ...
    if (direction > 0 ? t0 < t1 : t0 > t1) t0 += direction * tau3;
  }
  for (var point, t = t0; direction > 0 ? t > t1 : t < t1; t -= step) {
    point = spherical([cosRadius, -sinRadius * cos2(t), -sinRadius * sin2(t)]);
...
function circleRadius(cosRadius, point) {
  ...
  return ((-point[2] < 0 ? -radius : radius) + tau3 - epsilon5) % tau3;
}
function circle_default() {
  var center2 = constant_default5([0, 0]), radius = constant_default5(90), precision = constant_default5(2), ring, rotate, stream = { point };
...
function polygonContains_default(polygon, point) {
  var lambda = longitude(point), phi2 = point[1], sinPhi = sin2(phi2), normal = [sin2(lambda), -cos2(lambda), 0], angle2 = 0, winding = 0;
  var sum2 = new Adder();
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m3; ++j, lambda04 = lambda12, sinPhi03 = sinPhi1, cosPhi03 = cosPhi1, point0 = point1) {
      ...
      sum2.add(atan2(k * sign2 * sin2(absDelta), cosPhi03 * cosPhi1 + k * cos2(absDelta)));
  ...
  return (angle2 < -epsilon5 || angle2 < epsilon5 && sum2 < -epsilon22) ^ winding & 1;
}
...
function compareIntersection(a3, b) {
  return ((a3 = a3.x)[0] < 0 ? a3[1] - halfPi2 - epsilon5 : halfPi2 - a3[1]) - ((b = b.x)[0] < 0 ? b[1] - halfPi2 - epsilon5 : halfPi2 - b[1]);
}
  ...
  function clipLine(stream) {
    ...
    return {
      ...
      point: function(lambda, phi2) {
        var point1 = [lambda, phi2], point2, v2 = visible(lambda, phi2), c5 = smallRadius ? v2 ? 0 : code(lambda, phi2) : v2 ? code(lambda + (lambda < 0 ? pi2 : -pi2), phi2) : 0;
...
function contains_default2(object2, point) {
...
function interpolate_default(a3, b) {
  var x06 = a3[0] * radians, y06 = a3[1] * radians, x12 = b[0] * radians, y12 = b[1] * radians, cy0 = cos2(y06), sy0 = sin2(y06), cy1 = cos2(y12), sy1 = sin2(y12), kx0 = cy0 * cos2(x06), ky0 = cy0 * sin2(x06), kx1 = cy1 * cos2(x12), ky1 = cy1 * sin2(x12), d = 2 * asin(sqrt(haversin(y12 - y06) + cy0 * cy1 * haversin(x12 - x06))), k = sin2(d);
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b0, c0, x12, y12, lambda12, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a3 = a0 + a1, b = b0 + b1, c5 = c0 + c1, m3 = sqrt(a3 * a3 + b * b + c5 * c5), phi2 = asin(c5 /= m3), lambda22 = abs3(abs3(c5) - 1) < epsilon5 || abs3(lambda04 - lambda12) < epsilon5 ? (lambda04 + lambda12) / 2 : atan2(b, a3), p = project(lambda22, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x06, dy2 = y22 - y06, dz = dy * dx2 - dx * dy2;
      if (dz * dz / d2 > delta2 || abs3((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
        resampleLineTo(x06, y06, lambda04, a0, b0, c0, x22, y22, lambda22, a3 /= m3, b /= m3, c5, depth, stream);
  ...
  return function(stream) {
    ...
    function linePoint2(lambda, phi2) {
      ...
      resampleLineTo(x06, y06, lambda04, a0, b0, c0, x06 = p[0], y06 = p[1], lambda04 = lambda, a0 = c5[0], b0 = c5[1], c0 = c5[2], maxDepth, stream);
...
function conicProjection(projectAt) {
  var phi02 = 0, phi12 = pi2 / 3, m3 = projectionMutator(projectAt), p = m3(phi02, phi12);
...
function mercatorProjection(project) {
  ...
  function reclip() {
    var k = pi2 * scale2(), t = m3(rotation_default(m3.rotate()).invert([0, 0]));
...
function conicConformalRaw(y06, y12) {
  ...
  function project(x3, y3) {
    if (f > 0) {
      if (y3 < -halfPi2 + epsilon5) y3 = -halfPi2 + epsilon5;
    } else {
      if (y3 > halfPi2 - epsilon5) y3 = halfPi2 - epsilon5;
    }
...
function pack_default() {
  ...
  pack.padding = function(x3) {
    return arguments.length ? (padding = typeof x3 === "function" ? x3 : constant_default6(+x3), pack) : padding;
  };
  return pack;
}
...
function contains_default3(polygon, point) {
  var n = polygon.length, p = polygon[n - 1], x3 = point[0], y3 = point[1], x06 = p[0], y06 = p[1], x12, y12, inside = false;
  for (var i = 0; i < n; ++i) {
...
//# sourceMappingURL=chunk-EKE7HOPU.js.map
