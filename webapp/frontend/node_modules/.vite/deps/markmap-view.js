...
var Kt = (e, t, i) => {
  ...
  let o = n, h = n.relX, a = s, c = s.relX, l = true;
  for (; o && a; ) {
...
var it = class _it {
  constructor(t, i) {
    ...
    }, this.ensureView = this.ensureVisible, this.svg = t.datum ? t : select_default(t), this.styleNode = this.svg.append("style"), this.zoom = zoom_default().filter((n) => this.options.scrollForPan && n.type === "wheel" ? n.ctrlKey && !n.button : (!n.ctrlKey || n.type === "wheel") && !n.button).on("zoom", this.handleZoom), this.setOptions(i), this.state = {
...
`);
  ...
  async ensureVisible(t, i) {
    ...
    if (!n) return;
    const s = this.svg.node(), o = s.getBoundingClientRect(), h = transform(s), [a, c] = [
      n.state.rect.x,
  ...
  async centerNode(t, i) {
    ...
    if (!n) return;
    const s = this.svg.node(), o = s.getBoundingClientRect(), h = transform(s), a = (n.state.rect.x + n.state.rect.width / 2) * h.k + h.x, c = (n.state.rect.y + n.state.rect.height / 2) * h.k + h.y, l = {
      left: 0,
      right: 0,
      top: 0,
      bottom: 0,
   ...
   */
  async rescale(t) {
    const i = this.svg.node(), { width: n, height: s } = i.getBoundingClientRect(), o = n / 2, h = s / 2, a = transform(i), c = a.translate(
      (o - a.x) * (1 - t) / a.k,
      (h - a.y) * (1 - t) / a.k
    ).scale(t);
